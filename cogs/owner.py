...
class Owner(object):
    ...
    @commands.command('+gblack' , hidden=True, aliases=['+gblacklist'])
    @owner()
    async def block_guild(self, ctx, guild: int):
        ...
        json.dump(self.bot.guild_blacklist,
                  io.open('database/guild_blacklist.json', 'w', encoding='utf-8-sig'))
        await ctx.send(f'{self.bot.yes} Успешно.')


    @commands.command('-gblack', hidden=True, aliases=['-gblacklist'])
    @owner()
    async def unblock_guild(self, ctx, guild: int):
        """[RU] Разблокировать сервер.
        [EN] Unblock guild.
        """
        ...
        if not guild:
            return await ctx.send(f'{self.bot.no} Не удалось найти такой сервер.')
        try:
            self.bot.guild_blacklist.remove(guild.id)
            json.dump(self.bot.guild_blacklist,
                      io.open('database/guild_blacklist.json', 'w', encoding='utf-8-sig'))
            await ctx.send(f'{self.bot.yes} Успешно.')
        except:
            await ctx.send(f'{self.bot.no} Этот сервер не находится в моем черном списке.')
    ...
    async def shell(self, ctx, *, code: str):
        ...
        async with ctx.channel.typing():
            ...
            for string in code_strings:
                cmd = await self.bot.loop.run_in_executor(None, shell_, string)
                if cmd == '':
